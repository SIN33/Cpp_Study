...
DirectXEngine::DirectXEngine(unsigned int width, unsigned int height, std::wstring title)
	: title_(title)
	, windowWidth_(width)
...
// 初期化処理
void DirectXEngine::OnInit(HWND hwnd)
{
	LoadPipeline(hwnd);
	LoadAssets();
}
...
void DirectXEngine::LoadPipeline(HWND hwnd)
{
...
// D3D12Deviceの生成
void DirectXEngine::CreateD3D12Device(IDXGIFactory6* dxgiFactory, ID3D12Device** d3d12device)
{
...
void DirectXEngine::ThrowIfFailed(HRESULT hr)
{
	if (FAILED(hr))
	{
		// hrのエラー内容をthrowする
...
std::vector<UINT8> DirectXEngine::GenerateDummyTextureData()
{
